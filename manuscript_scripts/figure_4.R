...
# ---- reproducible seed ----
set.seed(123)
figures_folder <- "Manuscript_jan_2025/figures_02_05_2025"
...
celltype_props_normalized <- celltype_props_normalized %>%
  ...
  select(-Dataset) %>%
  rename("Cluster" = "predicted.celltype.l2") %>%
  group_by(Cluster) %>%
  mutate(Proportion = NormalizedCount / sum(NormalizedCount) * 100) %>%
  ungroup() %>%
...
abundance_degs <- abundance_degs %>%
  ...
  mutate(
    Count = ifelse(TotalNormalized < 0.01, 0, Count),  # set Count to 0 if under 1%,
    Percent = ifelse(TotalNormalized < 0.01, 0, Percent),
...
heatmap_plot <- ggplot(diff_df_complete, aes(x = TF, y = Cluster, fill = Diff)) +
  ...
  labs(x = paste0("loadings (", pc1_var_percent, "%)")) +  # Add dynamic x-axis title
  theme_minimal(base_size = 14) +
  theme(
    axis.text.y     = element_blank(),
    axis.ticks.y    = element_blank(),
...
ggsave(
  file.path(figures_folder, "combined_with_pca_and_heatmap.png"),
  final_plot,
  width  = 11,
  height = 8,
  dpi    = 300
...
single_TF_heatmap <- function(cell_type,
                              sort_by    = c("moderate","severe"),
                              top_n      = 20,
                              deltas_list,
                              global_max) {
  sort_by   <- match.arg(sort_by)
  sort_cond <- if (sort_by == "moderate") "Moderate" else "Severe"
  ...
  df_top <- df_long %>%
    ...
    mutate(
      ...
      Condition = factor(Condition, levels = c("Moderate","Severe"))
    )
  ...
  ggplot(df_top, aes(TF, Condition, fill = Delta)) +
    ...
    labs(title = paste0(cell_type, " (sorted by",sort_by,")"), x = "Regulon", y = NULL) +
    theme_minimal(base_size = 14) +
    theme(
...
file_name <- paste0("diff_tf_activity_",cell_type,"_", sort_by, "_", top_n, ".png")
ggsave(file.path(figures_folder,file_name), p, width = 8, height = 2)
