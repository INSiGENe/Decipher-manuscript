...
cntm <- config[[dataset_key]][["Connectome_analysis"]]

if (is.null(cntm)) {
  stop(paste("No 'pre_processing_for_analysis' section found for dataset:", dataset_key))
}

this_species <- cntm$this_species
case_condition <- cntm$case_condition
control_condition <- cntm$control_condition
dataset_path <- cntm$dataset_path
