...
parser = argparse.ArgumentParser(description="Load dataset and method parameters from config.json")
...
if dataset_name not in config:
    sys.exit(f"Error: Dataset '{dataset_name}' not found in config file.")

# Check if method exists for the dataset
if method_name not in config[dataset_name]:
    sys.exit(f"Error: Method '{method_name}' not found for dataset '{dataset_name}'.")

# Extract relevant variables for the chosen dataset and method
method_config = config[dataset_name][method_name]

# Assign variables (defaulting to None if missing)
#parameters
data_folder = method_config.get("data_folder")
this_species = method_config.get("species")
cluster_column = method_config.get("cluster_column")

print(f"Running {method_name} on {dataset_name} with variables: {var1}, {var2}, {var3}")
