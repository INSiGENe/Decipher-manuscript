...
plot_regulon_graph <- function(data, regulons) {
  ...
  # png("overlap_tatata.png", height = 40, width = 40, units = "cm", res = 300)
  # plot(
  #   g,
  #   vertex.label = V(g)$name,
  #   vertex.color = vertex_attrs$colors,
  #   vertex.size = vertex_attrs$sizes,
  #   vertex.label.cex = vertex_attrs$label_cex,
  #   edge.color = "gray"
  # )
  # dev.off()
}
...
#names(capped_regulons_all_clusters)
for(selected_ct in c("CD14_Mono","CD16_Mono")){
  ...
  # # Function to get PubMed count for a gene
  ...
  #col_names[!col_names %in% top_genes] <- ""
  ...
  ggsave(file.path("figures",filename),p,width = 12,height = 5,units="cm")
}
...
top_genes <- gene_counts_all_relevant_to_dataset %>%
  filter(Gene %in% genes) %>%
  arrange(desc(PubMed_Count)) %>%
  top_n(n = 30, wt = PubMed_Count) %>%
  pull(Gene)
...
#
#
# # Plot the binary heatmap with selective row names
# heatmap.2(
#   as.matrix(binary_matrix),
#   trace = "none",
#   col = c("white", "black"),
...
#   main = NA,
#   cexRow = 0.7,
#   labCol = col_names,  # Set selective row labels,
#   cexCol = 0.5
# )
...
my_palette <- c("white", "black", "red")
file_name <- paste(selected_ct,"tg_heatmaps.png")
file_dir <- file.path(data_path,"monodcs_to_cd4")
dir.create(file_dir)
png(file.path(file_dir,file_name))
# Plot the binary heatmap with selective row names
heatmap.2(
  binary_matrix_mat,
  trace = "none",
...
# doing something to speed up pubmed retrieval

library(dplyr)

# Combine all data frames in capped_regulons_all_clusters into one data frame
combined_df <- bind_rows(capped_regulons_all_clusters, .id = "cluster")

# Remove rows where 'source' contains 'sample' (e.g., "sample1", "sample2", etc.)
combined_df <- combined_df %>%
  filter(!grepl("^sample\\d+", source))

# Count the occurrences of each unique target across all data frames
target_counts <- combined_df %>%
  group_by(target) %>%
  summarise(count = n()) %>%
  arrange(desc(count))
...
table(target_counts$count >1)

genes_to_query <- target_counts$target[which(target_counts$count >1)]

# Apply the function to each gene and store the results in a data frame
gene_counts_all_relevant_to_datset <- data.frame(
  Gene = genes_to_query,
  PubMed_Count = sapply(genes_to_query, get_pubmed_count)
)
...
gene_counts_all_relevant_to_dataset <- data.frame(Gene = character(), PubMed_Count = integer(), stringsAsFactors = FALSE)


# Initialize the progress bar
pb <- progress_bar$new(
  format = "  Querying [:bar] :percent in :elapsed",
  total = length(genes_to_query),
  clear = FALSE,
  width = 60
)


# Loop through each gene and retrieve the PubMed count
for (gene in genes_to_query) {
  # Add a delay to avoid overloading the API
  Sys.sleep(0.5)

  # Try to get the count, with error handling
  count <- tryCatch({
    search_result <- entrez_search(db = "pubmed", term = gene)
    search_result$count
  }, error = function(e) {
    message(paste("Error retrieving data for gene:", gene))
    NA  # Assign NA if there is an error
  })

  # Append the result to the data frame
  gene_counts_all_relevant_to_dataset <- rbind(
    gene_counts_all_relevant_to_dataset,
    data.frame(Gene = gene, PubMed_Count = count, stringsAsFactors = FALSE)
  )
...
gene_counts_all_relevant_to_dataset <- readRDS("gene_counts_all_relevant_to_dataset.rds")

saveRDS(gene_counts_all_relevant_to_dataset,"gene_counts_all_relevant_to_dataset.rds")
