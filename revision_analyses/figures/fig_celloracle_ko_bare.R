...
make_panels <- function(csv, axis_lab, toward_lab, away_lab, tag_a, tag_b) {
  ...
  pa <- ggplot(d, aes(cos, tf, colour = dir)) +
    ...
    labs(x = paste0("Cosine alignment with ", axis_lab), y = NULL) + th +
  ...
  pb <- ggplot(dd, aes(g2, abs(cos), fill = g2)) +
    ...
    labs(x = NULL, y = "|cosine alignment|") + th + theme(panel.grid.major.y = element_line())
...
ggsave("fig_celloracle_ko_severity_bare.png", (sev$pa | sev$pb) + plot_layout(widths = c(2.2, 1)), width = 10.5, height = 6.2, dpi = 200, bg = "white")
ggsave("fig_celloracle_ko_severity_bare.pdf", (sev$pa | sev$pb) + plot_layout(widths = c(2.2, 1)), width = 10.5, height = 6.2)
ggsave("fig_celloracle_ko_vaccination_supp_bare.png", (vax$pa | vax$pb) + plot_layout(widths = c(2.2, 1)), width = 10.5, height = 6.2, dpi = 200, bg = "white")
ggsave("fig_celloracle_ko_vaccination_supp_bare.pdf", (vax$pa | vax$pb) + plot_layout(widths = c(2.2, 1)), width = 10.5, height = 6.2)
