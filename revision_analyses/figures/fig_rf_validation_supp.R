...
pb <- ggplot(lodo, aes(reorder(cl, med), lodo_r2, fill = dataset)) +
  ...
  labs(x = NULL, y = expression("Donor-held-out"~R^2), title = "b  Leave-one-donor-out validation by cell type") + th +
...
pc <- ggplot(cv, aes(method, cv_r2, fill = dataset)) +
  ...
  labs(x = NULL, y = expression("5-fold CV"~R^2), title = "c  Predictive accuracy by regressor") + th +
...
ggsave("fig_rf_validation_supp.png", p, width = 13, height = 10, dpi = 170, bg = "white")
ggsave("fig_rf_validation_supp.pdf", p, width = 13, height = 10)
cat("written\n")
