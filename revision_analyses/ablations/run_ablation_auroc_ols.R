...
# OLS ADD-ON (21 Aug 2026): same harness, adds OLSRegr; Decipher+RidgeRegr kept as like-for-like check.
# Output: results/ablation_auroc_ols.csv (dataset x method x threshold), resumable
...
out_csv <- file.path(out_dir, "results", "ablation_auroc_ols.csv")  # OLS add-on run (21 Aug 2026)
...
for (key in keys) {
  ...
  # --- OLSRegr (unpenalised lm; |beta_j|*sd(x_j); rank-deficient coefs -> 0) ---
  ols_list <- mclapply(clusters, function(cl) {
    pot <- potentials[[cl]]
    x <- t(pot)
    sds <- apply(x, 2, sd)
    edge_info <- dec_by_reg[[cl]] %>%
    ...
    contribs <- lapply(tfs, function(tf) {
      y <- as.numeric(regulons[[cl]][tf, colnames(pot)])
      fit <- tryCatch(suppressWarnings(lm.fit(cbind(1, x), y)), error = function(e) NULL)
      if (is.null(fit)) return(NULL)
      beta <- as.numeric(fit$coefficients)[-1]; beta[is.na(beta)] <- 0
      data.frame(interaction = rownames(pot), regulon = tf,
                 imp = abs(beta) * sds, stringsAsFactors = FALSE)
    })
    bind_rows(contribs) %>%
  ...
  v_ols <- setNames(ols_list, clusters)
  ...
  figpath <- file.path(out_dir, "figures_ols", key)
  dir.create(figpath, recursive = TRUE, showWarnings = FALSE)

  results_to_compare <- list(Decipher = v_decipher, RidgeRegr = v_ridge, OLSRegr = v_ols)
...
message("ABLATION AUROC OLS DONE")
